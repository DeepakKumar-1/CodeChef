...
public class NodeWithLargestValue {
    ...
    public static int nodesAtDistanceK(BinaryTreeNode<Integer> root, int target , int k){
        ...
        int leftDistance = nodesAtDistanceK(root.left, target, k);
        if(leftDistance != -1){
            if(leftDistance + 1 == k){
                System.out.print(root.val + " ");
            }else{
        ...
        int rightDistance = nodesAtDistanceK(root.right, target, k);
            if(rightDistance != -1){
                if(rightDistance+1 == k){
                    System.out.print(root.val + " ");
                }else{
                    nodesAtDistanceKDown(root.left, k - leftDistance - 2);
                }
